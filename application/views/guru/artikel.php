            ...
            <div class="modal-header">
                <h4 class="modal-title">Filter
                    <div class="pull-right">
                        <a href="<?php echo base_url(). "guru/artikel/index"; ?>" class="btn btn-primary">Atur Ulang</a>
...
<script type="text/javascript" language="javascript" >
    ...
    function searchFilter(page_num) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>guru/artikel/ajaxPaginationData/'+page_num,
    ...
    $('[name="artikel_title"]').bind("keyup", function(event, ui) {
        ...
        if(id == 0){
            $.ajax({
                type: "GET",
                data: 'term='+title,
                url: "<?php echo site_url('guru/artikel/slug'); ?>",
                cache: false,
                dataType:'json',
    ...
    function formDialog(id) {
        ...
        if(id > 0){
            ...
            $.ajax({
                type: "POST",
                data: 'id='+id,
                url: "<?php echo site_url('guru/artikel/ambildatabyid'); ?>",
                cache: false,
                dataType:'json',
                beforeSend: function () {
                    $('#loading_ajax').show();
    ...
    $(document).ready(function(){
        ...
        $('[name="artikel_topik"]' ).autocomplete({
            source: "<?php echo site_url('guru/artikel/data1/?');?>"
        });
        $('[name="artikel_tags"]' ).autocomplete({
            source: "<?php echo site_url('guru/artikel/data2/?');?>"
        });
        ...
        $('#_form').submit(function(e){
            ...
            $.ajax({
                type:'POST',
                data: form,
                url:'<?php echo base_url('index.php/guru/artikel/simpan') ;?>',
                dataType:'json',
                processData :false,
                contentType :false,
                cache :false,
    ...
    function submitHapus(id) {
        ...
        if(tanya){
            $.ajax({
                type:'POST',
                data: 'id='+id,
                url:'<?php echo base_url('guru/artikel/hapus') ;?>',
                cache: false,
                dataType:'json',
                success: function(hasil){
                    $('#Notifikasi').html('<p class="alert alert-success">'+hasil.pesan+'</p>');
