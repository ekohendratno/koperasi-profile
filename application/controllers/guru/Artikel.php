...
class Artikel extends CI_Controller{
    ...
    function index(){
        ...
        $this->template->load('template','guru/artikel',$data);
    }
    ...
    function ajaxPaginationData(){
        ...
        $config['base_url']    = base_url().'guru/artikel/ajaxPaginationData';
    ...
    function listOfQuery($params = array()){
        ...
        $this->db->where('artikel_author',$this->username);
    ...
    public function hapus()
    {
        ...
        else
        {
            ...
            $where = array(
                'artikel_id'=> $id,
                'artikel_author'=> $this->username
            );


            $artikel = $this->db->get_where('artikel',$where);
            foreach ($artikel->result() as $row) {
                ...
                if (!empty($row->artikel_foto) && file_exists(FCPATH . 'uploads/posts/' . $year . "/" . $month . "/" . $row->artikel_foto)) {
                    unlink(FCPATH . 'uploads/posts/' . $year . "/" . $month . "/" . $row->artikel_foto);
                }
            }
    ...
    public function ambildatabyid(){
        $id = $this->input->post('id');
        $users = $this->db->get_where('artikel', array('artikel_id'=>$id,'artikel_author'=> $this->username));
    ...
    function _jumlah_artikel($today){
        ...
        $ikut = $this->db->select('*')->from('artikel');
        $ikut = $ikut->where("artikel_ketegori","Blog");
        $ikut = $ikut->where("artikel_author",$this->username);
